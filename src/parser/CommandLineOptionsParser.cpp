...
namespace EasySoftCmdLine
{
    ...
    std::vector<KeyValueOption>& CommandLineOptionsParser :: Parse(int argc, char** argv)
    {
        try
        {
            ...
            _programOptions.push_back(EasySoftCmdLine::KeyValueOption (firstKey, argv[0]));
            for(int argCounter = 1; argCounter < argc; argCounter++)
            {
                ...
                if(CheckIsKey(argumentString))
                {
                    ...
                    if(CheckValueIsPresent(argumentString))
                    {
                        ...
                        _programOptions.push_back(EasySoftCmdLine::KeyValueOption (argumentString.substr(0, index), argumentString.substr(index + separator.size())));
                        continue;
                    }
                    if(argCounter + 1 < argc)
                    {
                        ...
                        {
                            _programOptions.push_back(EasySoftCmdLine::KeyValueOption (argumentString, nextArgumentString));
                            continue;
                        }
                    }
                    // 3. option contains only key and no value
                    _programOptions.push_back(EasySoftCmdLine::KeyValueOption (argumentString, std::string ()));
                }
            }
